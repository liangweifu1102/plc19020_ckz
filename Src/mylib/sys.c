...
void MY_NVIC_Init(u8 NVIC_PreemptionPriority, u8 NVIC_SubPriority, u8 NVIC_Channel) //, u8 NVIC_Group)
{
    ...
    temp = NVIC_PreemptionPriority << (4 - SYS_NVIC_Group);
    temp |= NVIC_SubPriority & (0x0f >> SYS_NVIC_Group);
    temp &= 0xf;                                             //取低四位
...
//设置栈顶地址
//addr:栈顶地址
void MSR_MSP(u32 addr) //u32 addr)
{
...
u8 Sys_Clock_Set(u32 plln, u32 pllm, u32 pllp, u32 pllq)
{
    ...
    else
    {
        ...
        /**
		AHB：0 HCLK 不分频；  8  2分频； 9 4分频  
		APB1：0 不分频； 4 2分频；5 4分频
		APB2：0 不分频； 4 2分频；5 4分频
		*/
        RCC->CFGR |= (0 << 4) | (5 << 10) | (4 << 13);                                            // ;;APB2 2分频.
        RCC->CR &= ~(1 << 24);                                                                    //关闭主PLL
...
#else
    MY_NVIC_SetVectorTable(NVIC_VectTab_FLASH_USER, 0x20000);
#endif

    SystemCoreClock = 120000000;
    MY_NVIC_PriorityGroupConfig(SYS_NVIC_Group);
}

/**
 * 禁止时钟
 *
 * @author xuzhenglim (2017-11-23)
 *
 * @param void
 */
void Stm32_Clock_DeInit(void)
{
    //复位时钟
    /* Set HSION bit */
