...
uint8_t KEYBRD_Decode(uint8_t *pbuf)
{
    ...
    //uint8_t *pbuf = (uint8_t *)keybd_report_data;
    uint8_t ix;
    uint8_t jx;
    uint8_t error;
    uint8_t output;
    ...
    if (nbr_keys_new == 1)
    {
        ...
        output = HID_KEYBRD_Codes[key_newest];
        if (shift == TRUE)
        {
            output = HID_KEYBRD_ShiftKey[output];
        }
        else
        {
            output = HID_KEYBRD_Key[output];
        }
...
{
    uint8_t output;
    if ((info->lshift > 0) || (info->rshift > 0))
    //if (shiftflag > 0)
    {
    ...
    //info_old = *info;
//  if ((info->lshift > 0) || (info->rshift > 0)) shiftflag = 1;
//  else shiftflag = 0;
    return output;
}
